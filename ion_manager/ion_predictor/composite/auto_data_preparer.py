...
def clean_df(original_df):
    ...
    parsed_df["Conductivity"] = np.log10(
        parsed_df["Conductivity"].astype(float))
    ...
    parsed_df["inorg_contain_ratio(wt)"] = parsed_df["inorg_contain_ratio(wt)"].astype(
        float)
...
def load_ion_excel(settings,
    ...
    if composite_df is None:
        composite_df = pd.read_excel(
            excel_path, sheet_name=composite_sheet_name)
...
def auto_df_load(compound_df, composite_df, settings):
    ...
    # load main database
    raw_df = process_composite_dataframe(
        composite_df, compound_parser, neural_calculator)
